...
\begin{command}{perturb}{}
	...
	\begin{poyexamples}
	    ...
	    \poyexample{perturb(resample:(50,terminals), iterations:10)}
            ...
            percents of terminals with replacement. Branch swapping is performed
...
\begin{command}{read}{}
	...
	\begin{poydescription} 
        ...
        ``Sys\_error (``No such file or directory'')''.  \poycommand{read} automatically detects the type of the 
	...
	\begin{arguments}
	    ...
	    \begin{argumentgroup}{Data file types}
            ...
            \begin{statement}
                ...
                signifies missing data, a gap, or any nucleotide base may occur in that matrix position.
       ...
       \argumentdefinition{breakinv}{\obligatory{(\poystring, \poystring, {\optionall {orientation:\poybool})}}}
            ...
            \poyargument{custom\_alphabet} data type.
 ...
 \argumentdefinition{custom\_alphabet}{\obligatory\poystring list,cm\obligatory \poystring,{\optionall 
 {{level{\obligatory (\poyint, \poylident)}, init3d{\obligatory \poybool},tiebreaker{\obligatory \poylident},orientation:\poybool})}}}
            ...
            The \texttt{cm} refers to the custom-alphabet matrix that contains two parts:
	...
	\smallskip
	Four optional arguments of \poyargument{custom\_alphabet} are possible: \poyargument {level}; \poyargument{init3d}; 
	\poyargument{tiebreaker}; and \poyargument{orientation}.  \poyargument {level} determines the heuristic 
	level of the median sequence calculation.  For this optional argument, the user must define the level itself, 
	as specified by the \poyint, and the keep method, as specified by the \poylident (\texttt{first}, \texttt{last}, or 
	\texttt{at\_random}.  The default \poyargument{level} is \texttt{2} and keep method is \texttt{first}.  Both \poyargument{init3d} and 
	\poyargument{orientation} require an obligatory boolean value, namely \texttt{true} or \texttt{false}.
	Both these arguments allow the user to specify the orientation of custom-defined alphabet
	characters. The tilde (``$\sim$'') symbol preceding an alphabet character indicates
	the negative orientation. The default option is \texttt{true}. \poyargument{init3d} initiates a 3D matrix, 
	but the user should be aware that this option can consume a lot of memory.  The \poylident values for
	\poyargument{tiebreaker} are \texttt{first}, \texttt{last}, and \texttt{at\_random}.  If \texttt{first} is chosen, 
	then the ties are broken (if the number of equally costly states is greater than the level number) by 
	choosing the first median state examined; if \texttt{last}, the last state, and if \texttt{at\_random} then at random.
          The default choice method is \texttt{first}.

	\poycommand{custom\_alphabet} can be transformed into \poycommand{breakinv} using ~\ccross{transform}.}
	 {customalphabet}
        
        \argumentdefinition{genome}{\obligatory{(\poystring list)}}
            {Specifies that the data listed in the string argument are
            multichromosomal nucleotide sequences with an at sign (``$@$'')  
	...
	\begin{poyexamples}
        ...
        \poyexample{read(custom\_alphabet:("my\_data",cm:"alphabet",tiebreaker:last, orientation:false))}
        {Reads the first file, \texttt{my\_data}, containing data in the format of a custom
        alphabet, which is defined in the \texttt{cm} file, \texttt{alphabet}. By default, the
...
\begin{command}{rename}{}
	...
	\begin{statement}
        ...
        executed {\bf \emph{before}} importing the data files (see command \nccross{read}{read})
...
\begin{command}{report}{}
	...
	\argumentdefinition{consensus}{\optional{\poyint}}
            ...
            \argumentdefinition{graphtrees}{\optional{collapse\optional{\poybool}}}
                ...
                previous or next tree respectively. If no graphical support is available, the output 
	     ...
	     {}
            ...
            \argumentdefinition{supports}{\optional{argument}}
                ...
                \poycommand{report(supports:bremer:"file.txt")}, or a list of
                strings encloed in parentheses (as in
                \poycommand{report(supports:bremer:("file1.txt", "file2.txt"))}.
	...
	\end{argumentgroup}
        ...
        \nccross{transform}{transformcommand}.}
           ...
           \argumentdefinition{missing}{\obligatory{\poyint}}
                ...
                than that of the defined value are included in the analysis (compare with \nccross{notmissing})}
                {missing}
	...
	\begin{arguments}
        ...
        \begin{argumentgroup}{Application settings}
            ...
            \argumentdefinition{log}{\obligatory{\poystring}}
                ...
                slow down the execution time to some extent, as the application has
            ...
            \argumentdefinition{nolog}{}
                ...
                above. Unless specified, no log is set my the program.}
                {nolog}
...
%                        
        ...
        \begin{argumentgroup}{Cost calculation}
            ...
            \argumentdefinition{iterative}{\obligatory{\poylident:\optional \poyint}}
                ...
                time. When \poyargument{approximate}, the execution time footprint is much
	 ...
	 \begin{statement}
        ...
        \begin{argumentgroup}{Likelihood Optimization}
            ...
            \argumentdefinition{opt:coarse}{\optional {\poyint}}
                ...
                magnitude of a full, exhaustive search). By default, 
    ...
    \begin{poyexamples}
    	\poyexample{set(history:10000, seed:45, log:"mylog.txt")}
            ...
            interface to 10,000 lines, sets the random number generator to 45,
...
\begin{arguments}
  ...
  \begin{argumentgroup}{Reroot order}
	...
	\begin{poyexamples} 
        ...
        \poyexample{swap(spr, optimize:(model:(never), branch:(join\_region)))}
            {Submits the current trees to a round of SPR swapping.  Following each round of SPR,
            the model is never optimized, but the branches are optimized on each join.}
...
\begin{command}{transform}{transformcommand}
	...
	\begin{arguments}
     ...
     \argumentdefinition{do}{}
          {Synonymous with \poyargument{directoptimization}.}
          {do}
      ...
      \argumentdefinition{level}{\obligatory{(\poyint}, \poylident)}
          {The integer argument specifies the heuristic level in median sequence calculation.  
           ...
           [Note: Levels greater than the default levels, i.e.  1 for \texttt{amino acid} sequences 
           and 2 for \texttt{custom\_alphabet} characters, can consume large amounts of memory.] }
          {level}
        ...
        \argumentdefinition{partitioned}{\obligatory{\poylident}} %or is it \optional ?%
          ...
          \argumentdefinition{prealigned}{}
                ...
                at the \texttt{5'} and/or \texttt{3'} ends of the sequence if data is missing).}
                {prealignedtransform}
	...
	\end{statement}
            ...
            [Note: previous versions of \texttt{poy}, had
            the cost of an indel set to \texttt{2} (i.e. \poyargument{tcm:(1,2)}).]}
            {transformtcm}
	...
	\end{statement}
        ...
        \argumentdefinition{trailing\_deletion}{\obligatory{\poystring /(\poyint list)}}
            ...
            {Changes the cost of specified characters by a constant or absolute value 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
	   ...
	   \argumentdefinition{annotate}{\obligatory{(\poylident,\poyfloat,\poyfloat,\poyfloat,\poyfloat)}}
                ...
                of a sequence shared by two or more genomes and are without 
            ...
            \poyargument{custom\_alphabet} data type. Users specify 
                ...
                (\poyargument{median}), as well as defining \poyargument{orientation}.} 
                {breakinv}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        \begin{argumentgroup}{Likelihood transformation methods}
            ...
            \argumentdefinition{likelihood}{(\optional{argument list})}
                ...
                likelihood characters. All arguments are optional expect for model or model selection, 
            ...
            \argumentdefinition{elikelihood}{(\optional{argument list})}
                ...
                across branches of the tree. As with the normal likelihood command,
            ...
            \begin{statement}
                ...
                MPL can be implemented on static characters.
            \end{statement}
                    ...
                    each node and score the topology.
                       ...
                       \argumentdefinition{mal}{}
                        ...
                        \argumentdefinition{mpl}{}
                            ...
                            \cite{zou2011} and that the method may not be consistent
...
\begin {argumentgroup} {Gap treatment}
		 %   \argumentdefinition{gap}{\obligatory{\poylident}}
                    ...
                    indels on static likelihood characters follows McGuire et al. 
                    ...
                    and Liu et al \cite{liu2009rapid,liusateII2012} for an iterative approximation of
                    simultaneous inference under the MAL criterion.

                 \argumentdefinition{gap} {\obligatory character}
                            {Specifies that atomic indels are a character
        		  ...
        		  \argumentdefinition {gap} {\obligatory coupled}
                            ...
                            to scale their mean rate to one.}
                            {coupled}
...
\begin {argumentgroup} {Model choice}
		...
		theoretic approaches to model selection are provided.
                These include the Akaike Information Criteria (AIC)
                \cite{akaike1973}, the second order information criteria (AICc)
                \cite{sugiura1978}, and the Bayesian Information Criteria (BIC)
                \cite{schwarz1978}.
                ...
                on the information criteria selected) to be kept in memory. A
                ...
                with a weight given by,
                    ...
                    \argumentdefinition{aic}{\optional {\poystring}}
                        {Uses the AIC (Akaike Information Critera) to determine
...
\begin{argumentgroup}{Rate distributions}
          %  \argumentdefinition{rates:(TYPE)}{}
                ...
                \cite{gu1995}. This is only useful when more than two taxa are in the dataset.
