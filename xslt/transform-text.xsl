...
<xsl:key name="alphabet" match="Element" use="@Value" />
	...
	<xsl:template match="Diagnosis/Forest/Tree">
							...
							<xsl:if test="( $descendantValue != $ancestorValue ) and @Class='Final'">
								<xsl:choose>
									...
									<xsl:otherwise>
										...
										<xsl:text>&#10;&#x09;ancestor state      : </xsl:text>
										<xsl:value-of select="translate($ancestorValue,'_',' ')"/>
										<xsl:text>&#10;&#x09;descendant state    : </xsl:text>
										<xsl:value-of select="translate($descendantValue,'_',' ')"/>
										<xsl:text>&#10;&#x09;type                : non-additive character</xsl:text>
										<xsl:text>&#10;&#x09;cost                : </xsl:text>
										<xsl:value-of select="@Cost"/>
										<xsl:text>&#10;&#x09;definite            : </xsl:text>
			...
			<xsl:text>&#10;___________________________________________________________________________________</xsl:text>
