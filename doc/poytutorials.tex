...
\item \texttt{set(root:"taxon1")} The outgroup (\texttt{taxon1}) is designated by the \poyargument{root}, so that all the reported trees have the desired polarity.
...
\item \texttt{read(annotated:("genInv2"))} This command imports the annotated chromosomal sequence file \texttt{genInv2}. The argument \poyargument{annotated} specifies the characters. 
