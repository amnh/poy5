...
let () = SadmanOutput.register "FloatSequence" "$Revision: 3649 $"
...
(*                        i j acst icst dcst temp_min (int_of_dir temp_dirs);*)
    ...
    let ukkonen_align_2 ?(uk_min=0) ?(uk_max=max_int) (mem:floatmem) x y m tx ty =
        ...
        if (Array.length mem) < lenX then
            failwithf "FloatSequence.MPLAlign; not enough memory in scratch space; Has %d, requires %d"
...
let test_all alignments channel seq1 seq2 bl1 bl2 model =
    ...
    let mpl_cost,mpl_opt_cost,mpl_seq1,mpl_seq2,mpl_median =
    ...
    if alignments then begin
        Printf.fprintf channel "%a %a\n%!" (pp_seq model) mpl_seq1 (pp_seq model) cmpl_seq1;
        Printf.fprintf channel "%a %a\n%!" (pp_seq model) mpl_seq2 (pp_seq model) cmpl_seq2;
    end;
    Printf.fprintf channel "%f %f %f %f %f %f\t%a %a\n%!"
                   mal_cost     mpl_cost     cmpl_cost
                   mal_opt_cost mpl_opt_cost cmpl_opt_cost
                   (pp_seq model) mpl_median (pp_seq model) cmpl_median
