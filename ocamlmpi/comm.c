...
/* $Id: comm.c 2 2004-06-22 15:40:20Z andres $ */
...
value caml_mpi_intercomm_create (value local_comm, value local_leader, value remote_comm, value remote_leader, value tag)
{
  MPI_Comm newcomm;
  MPI_Intercomm_create(Comm_val(local_comm), Int_val(local_leader), 
      Comm_val(remote_comm), Int_val(remote_leader), Int_val(tag), &newcomm);
  return caml_mpi_alloc_comm(newcomm);
}
