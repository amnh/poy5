...
let () = SadmanOutput.register "IntSpec" "$Revision: 3459 $"
...
let to_formatter t : Xml.xml Sexpr.t =
    `Single (Xml.KolSpecs.int_set, 
    [(Xml.KolSpecs.min, `Int t.min); 
    (Xml.KolSpecs.max, `Int t.max);
    (Xml.KolSpecs.prob, `Float (decoder t))], `Empty)
