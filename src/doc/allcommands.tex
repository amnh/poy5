\begin{command}{build}{buildcommand}
	...
	\begin{arguments}
        ...
        \argumentdefinition{all}{}
            ...
            the number of  trees can be specified by the argument \poyargument{trees}.}
            {allbuild}
...
\begin{command}{calculate\_support}{calculatesupport}
	...
	\begin{poydescription} 
	  ...
	  The \poycommand{calculate\_support} command does not output support values by default. The output of
            support values must be requested using the command~\ccross{report}. This is particularly important for Jackknife and Bootstrap support values, as these sampling techniques
	...
	\end{statement}          
        ...
        \begin{argumentgroup}{Search strategy}
	     ...
	     \argumentdefinition{build}{}
             ...
             is selected (at random? the first one?}. If \poyargument{build} is
   ...
   % \poycommand{calculate_support (jackknife: (remove: X, resample: 5))}
    
    %\poycommand{calculate_support (bootstrap: (remove: X, resample: X))}
...
\begin{command}{transform}{transformcommand}
	...
	\begin{arguments}
        ...
        \argumentdefinition{j}{\poylident}
	...
	\end{statement}
        ...
        \argumentdefinition{trailing\_deletion}{\obligatory{\poystring /(\poyint list)}}
            ...
            \argumentdefinition{level}{\obligatory{\poyint}}
        ...
        \begin{argumentgroup}{Likelihood transformation methods}
            ...
            \argumentdefinition{likelihood}{\obligatory{(model,rates,priors,gapmode,cost)}}
                {Transforms the characters to either static or dynamic likelihood
                characters using the specified model.}
                {likelihood}

            \begin{statement}
                Dynamic likelihood for unaligned sequence data is
                implemented only as most parsimonious likelihood (MPL) (Barry and
                Hartigan, 1987). Both maximum average likelihood (MAL) (Felsenstein,
                1978) and most parsimonious likelihood are implementable on
                static characters.
            \end{statement}

            \begin{statement}
                It is important to note that dynamic likelihood characters
                necessarily include gaps/indels as a state in the character
                alphabet. See information for \poycommand{gapmode()}, below.
            \end{statement}
            ...
            \argumentdefinition{model}{}
                ...
                employ globally homogeneous models (jayaswal2005), which
                ...
                during the transformation to likelihood characters. Valid models are:

                \begin{description}

                    \argumentdefinition{jc69/neyman}{}
                        {The model of Jukes and Cantor (1969). A single rate
                        class. All equilibrium frequencies are forced to be
                        equal. This model will work for any size alphabet.}
                        {jc69}

                    \argumentdefinition{f81}{}
                        {The model of Felsenstein (1981). A single rate class,
                        proportional to the equilibrium frequencies. This model
                        will work for any size alphabet.}
                        {f81}

                    \argumentdefinition{k2p/k80}{}
                        {The model of Kimura (1980). Transitions and
                        transversions are constrained to have separate rate
                        classes. All equilibrium frequencies are forced to be
                        equal.}
                        {k2p}

                    \argumentdefinition{f84}{}
                        {The model of Felsenstein (1984). Rates are constrained
                        as in k2p, but are also proportional to the equilibrium
                        frequencies.}
                        {f84}

                    \argumentdefinition{hky85}{}
                        {Three rate classes: transversions (purine /
                        pyrimidine), and independent classes for purine-purine
                        and pyrimidine-pyrimidine transitions. Rates are also
                        proportional to equilibrium frequencies.}
                        {hky85}

                    \argumentdefinition{gtr}{}
                        {The model of Tavar\'{e} (1986). The most general
                    ...
                    \argumentdefinition{custom}{}
                        ...
                        Alfaro and Huelsenbeck (2004) note that many of the
                        unnamed variants of the 256 possible GTR submodels
            ...
            \argumentdefinition{rates}{}
                {Specifies the nature of among-site rate variation. Rate
                ...
                variation are the gamma distribution ($\Gamma$) and this
                distribution with an additional invariable rate class, the theta
                distribution ($\Theta$).
                
                It should be noted that although many researchers apply both a
                gamma and theta in likelihood analyses, the parameter values
                inferred are correlated (Sullivan, 1999).  It has therefore been
                recommended that a proportion of invariant sites be
                ``pseudo''-estimated by increasing the number of discrete
                categories for the gamma distribution to account for very low
                rate character groups. The mean of each class is used to define
                the rate for the discrete category.

                \begin{description}

                    \argumentdefinition{gamma}{}
                        {Apply a gamma distribution, $\gamma(\alpha,\beta)$, of
                        rate variation across characters. The mean rate for the
                        distribution is set to 1.0 ($\alpha = \beta$), thus one
                        addtional parameter is estimated.}
                        {gamma}

                    \argumentdefinition{theta}{}
                        {Apply a gamma distribution with an additional
                        invariable rate class optimized to be the proportion of
                        invariant sites in the data.}
                        {theta}
                ...
                \argumentdefinition{priors}{}
                    ...
                    \begin{description}
                        ...
                        \argumentdefinition{equal}{}
                            {Constrains each of the equilibrium frequencies to be
                            $\frac{1}{r}$, where $r$ is the character alphabet size.}
                            {equal}

                        \argumentdefinition{estimate}{}
                            {Uses observed frequencies of the characters to
                            approximate the ML-optimized equilibrium frequencies.
                            Under dynamic likelihood, the indel equilibrium
                            frequency is estimated to be the minimum number of
                            indels required to align all the sequences, $\pi_{gap} =
                            \sum_i=0^n ||S_{\hat{i}}|| - ||S_i||$, where
                            $S_{\hat{i}}$ is the longest sequence.}
                            {estimate}

                            \argumentdefinition{given:(FLOAT LIST)}{}
                            {Uses equilibrium frequencies given as a comma-separated list specified by the user.}
                            {given}

                    \end{description}}
                {priors}

                \argumentdefinition{cost}{}
                    {Specifies the variant of relative likelihood (\textit{sensu}
                    Steel and Penny [2000]) used by POY to calculate the costs at
                    ...
                    \begin{description}
                        ...
                        \argumentdefinition{mal}{}
                            ...
                            likelihood (Felsteinstein, 1978), in which the
                        ...
                        \argumentdefinition{mpl}{}
                            ...
                            (Barry and Hartigan, 1987), in which the likelihood is
                            ...
                            (Mossel and Steel, 2009).}
                            {mpl}

                    \end{description}}
                    {cost}

                \argumentdefinition{gapmode}{}
                    {Defines how atomistic gaps (indels) are treated in likelihood analysis. When implemented, the POY approach to indels on static likelihood characters follows McGuire et al. (2001), but expands the implementation outside the F84 model. POY is unique among likelihood implementations both by enabling indel parameterization for any reversible, stationary model applied to static characters and, more crucially, for implementing dynamic likelihood for truly simultaneous alignment and topology inference. Indel-aware models are scarce, but see Rivas and Eddy (2008), Redelings and Suchard (2005, 2007) for alternative parameterizations, and Liu et al. (2009) for an iterative approximation of simultaneous inference under the MAL criterion.

                    Note that under dynamic likelihood, only the coupled option is implemented. Options are:

                    \begin{description}

                        \argumentdefinition{missing}{}
                        ...
                        \argumentdefinition{independent}{}
                            {Specifies that atomistic indels are a character,
                            with rates of nucleotide-to-indel substitution
                            constrained to be estimated separately. For example,
                            the Q-matrix

                            \begin{equation*}
                                Q_{independent} =
                                    \begin{bmatrix}
                                        -3 \alpha - \chi & \alpha & \alpha & \alpha & \chi \\
                                        \alpha & -3 \alpha - \kappa & \alpha & \alpha & \kappa \\
                                        \alpha & \alpha & -3 \alpha - \mu & \alpha & \mu \\
                                        \alpha & \alpha & \alpha & -3 \alpha - \gamma & \gamma \\
                                        \chi & \kappa & \mu & \gamma & - \chi - \kappa - \mu - \gamma
                                    \end{bmatrix}
                            \end{equation*}

                            specifies a Jukes-Cantor matrix augmented to
                            estimate each nucleotide-to-indel substitution as a
                            unique rate separate from nucleotide-to-nucleotide
                            substitutions. In accordance with other likelihood
                            implementations, the diagonal entries of the
                            Q-matrix are normalized by the trace to scale their
                            mean rate to one.}
                            {independent}
		...
		\end{statement}
 	        ...
 	        \argumentdefinition{circular}{\obligatory{\poybool}} 
		      ...
		      \argumentdefinition{median}{\obligatory{\poyint}}
               ...
               \argumentdefinition{median\_solver}{\obligatory{\poyint}}
