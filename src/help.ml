let index = [
	("build",("Builds Wagner trees†[]. Building multiple trees with a randomized addition of terminals allows for
the evaluation of many more possible tree topologies and generates a diversity of trees for subsequent analysis. 
The arguments of the command build specify the number of trees to be generated and
the order in which terminals are added during a single tree building procedure. During tree
building, POY5 reports in the Current Job window of the ncurses interface
which of the terminal addition strategies (e.g. as_is or randomized) is currently used.","build(20) 
Builds 20 Wagner trees randomizing the order of terminal
addition (note that because the argument randomized 
...
set. For the total number of trees, rather than simply specifying 20,
the label trees is used. The verbose version might be desirable
to improve the readability of the script.build(all:30) 
Builds 30 Wagner trees, trying all possible addition positions for all terminals.	build(15, as_is) 
...
file \"cstree.tre\".build(trees:100,optimize(model:max_count:5,branch: 
all_branches)) 
...
and Bootstrap†[]) and Bremer support†[, ]. The Jackknife and Bootstrap support values are computed as
...
calculate_support command are optional and
their order is arbitrary. For examples of scripts implementing support measures see 
tutorials 5.4, 5.5 and 5.6.","calculate_support(bremer) 
Calculates Bremer support values by performing
independent searches for every node for every tree in memory. 
This is equivalent to executing calculate_support(), the default setting.calculate_support(bremer, build(trees:0), swap(trees:2)) 
...
is equivalent to calculate_support(bootstrap:5, 
build(trees:1,
...
these trees using tbr, and keeping up to 5 minimum-cost tree in the
...
ncurses interface, and to the standard output (stdout in the flat
interface)."));
	("exit",("Exits a POY5 session. This command does not accept any argument.
exit is equivalent to the command quit.","exit() 
Quits the program."));
...
between pairs of trees. Only one pair of trees is evaluated during a single iteration.","fuse(iterations:10, replace:best, keep:100, swap()) 
...
swap. The entire procedure is repeated nine more times.fuse(optimize:(model:never, branch:join_region)) 
...
each round.fuse(swap(constraint)) 
...
window of the ncurses interface or to the standard error of the flat interface.","inspect(\"initial_search.poy\") 
...
transform (cost of substitution 4 and cost of indel 3).
...
performed using TBR and keeping up to 5 optimal trees in each iteration.perturb(transform(static_approx), ratchet:(0.2,5), iterat-
...
‚ÄúSys_error (‚ÄúNo such file or directory‚Äù)‚Äù. read automatically detects the type of the 
input file. This command can also use wildcard expressions (such as *) to
refer to multiple files in a single step. For example, read(\"*.fas*\") 
imports all files of the FASTA format in the current directory (in this case 
this will include files that end in both .fas and .fasta. Moreover, 
importing all files that begin with the filename BAP is achieved by typing 
read(\"BAP.*\"). Specifying a filename(s) is obligatory: 
an empty argument string, read(), results in no data being 
read by POY5. The list of imported files and their content
can be reported on screen or to a file using report(data).","read(\"/Users/andres/data/test.txt\") 
...
a.FASTA.read(annotated:(\"ch1.txt\",\"ch2.txt\"),chromosome:(\"ch3.txt\")) 
...
the first will be chosen. Any tilde (‚Äú‚àº‚Äù) symbols in the data file will be ignored. read(custom_alphabet:(\"CA_run1\",tcm:(\"alphabet\"),level:2,
tiebreaker:last)) 
...
Reads the prealigned data file 18s.aln which was generated from the nucleotide file 18s.FASTA
	using the the transformation costs 1 for substitutions and 2 for indels.
...
(in the current POY5 session) command swap. Otherwise, it has no effect.
	...
	The trees imported by recover are appended to those currently
	stored in memory.
	
	Note that using recovered trees is not intended for temporary storage of trees.
	It is useful only as an intermediary operation in a given part of a POY5 session. When
	other commands that require clearing memory are executed (such as
	build, calculate_support, or another
	swap),
	the trees stored by recover can no longer be retrieved.","recover() 
...
alignments, trees, as well as other miscellaneous arguments.","TOC subsubsection id=sec138 Defaults"));
...
There are no default settings of run.","run(\"script1\", \"script2\") 
...
best trees to subsequent iterations of the command (see the example for parallel processing).","search(hits:100, target_cost:385, max_time:1:12:13) 
...
fragment 0 contained in the file chel.aln.
...
Same as the previous, but using the exact algorithm.set(opt:exhaustive:3) 
...
the name is used only to recover the stored state using the command use.","store(\"initial_tcm\") 
transform(tcm:(1,1)) 
use(\"initial_tcm\") 
...
presumes that the file bleh is located in the current working directory.	
...
probability of keeping a suboptimal tree of 3 steps longer = 1 / (2.0 + 3) = 0.2.swap(spr, all, optimize:(model:never, branch:join_region)) 
...
edges on either side of the join site) are optimized.swap(recover, timedprint:(5, \"timedprint.txt\")) 
...
command, use, recovers the initial state stored under the
name initial_tcm."));
	("version",("Reports the POY5 version number in the output window of the ncurses
interface, or to the standard error in the flat interface.","version ()"));
	("wipe",("Eliminates the data stored in memory (all character data, trees, etc.).","wipe ()"));
]