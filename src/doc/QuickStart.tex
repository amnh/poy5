...
\begin{quote}
\commandstyle{mpirun -np 8 $\sim$/POY\_bins/poy5a-mpi mol.poy}\\
\end{quote}
...
\setlength{\parindent}{0.5cm}        
	...
	\item [Chromosome and Mauve Parameters] \emph{Chromosome} characters are multi-locus 
		...
		Coverage}) are employed by the Mauve aligner to find regions of homology or synteny blocks 
		 between chromosomes (see~\nccross{annotate}{chromosomeannotate} within the 
		 command \poycommand{read}). Default values for these parameters are provided in the \emph{GUI}.
		...
		{chromosomelocusindel}  within the command \poycommand{transform}). Rearrangements of 
		...
		Siepel, Simpllk} and \emph{Vinh} (see~\nccross{median\_solver}{chromosomemediansolver} 
		within the command 	\poycommand{transform}).
