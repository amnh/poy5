...
(*        List.iter output_branch_data assoc_sets branch_assoc;*)
        fo "END;@.@]";
        ()
    end else begin
        fo "@[BEGIN POY;@]@.";
    ...
    let output_name (position,prev) code =
        ...
        let labels = 
            let string_labels, number_labels = 
                match Hashtbl.find data.Data.character_specs code with
                | Data.Dynamic _ | Data.Set -> assert false
                | Data.Static spec ->
...
let to_nexus data filename = 
    ...
    let terminals_not_ignored = 
        All_sets.IntegerMap.fold (fun code name acc ->
            if All_sets.Strings.mem name data.Data.ignore_taxa_set then acc
            else All_sets.IntegerMap.add code name acc)
        data.Data.taxon_codes All_sets.IntegerMap.empty 
    in
    ...
    let output_characters_blocks () : unit =
        ...
        let output_dynamic_homology character_code = 
            ...
            | Data.Static _ | Data.Set -> assert false
            | Data.Dynamic spec ->
                    fo "@[BEGIN UNALIGNED;@]@.";
                    fo "[CHARACTER NAME: ";
