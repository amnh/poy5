...
module MakeNormal (Node : NodeSig.S) (Edge : Edge.EdgeSig with type n = Node.n) 
    ...
    let single_wagner data tabu_mgr wmgr cg nodes = 
    ...
    let wagner data tabu_mgr cg nodes =
        single_wagner data tabu_mgr single_wagner_search_manager cg nodes

    let randomized_single_wagner data tabu_mgr randomize wmgr cg = 
        let nodes = randomize () in
        single_wagner data tabu_mgr wmgr cg nodes
    ...
    * original addition sequence as generated by [b ()].*)
    let max_n_wagner data threshold tabu_mgr data_generator cg n =
    ...
    let max_n_dg_p_wagner data threshold tabu_mgr data_generator cg n p : phylogeny Sexpr.t =
        match p, n with
        | 0, _ -> `Empty
        | 1, 1 -> 
                let st = Status.create "Building Wagner Tree" None "" in
                let res = rand_wagner data tabu_mgr data_generator cg in
                Status.finished st;
                res
        | 1, n -> 
                let st = Status.create "Building Wagner Tree" None "" in
                let res = max_n_wagner data threshold tabu_mgr data_generator cg n in
                Status.finished st;
                res
        | p, _ ->
              let builder cnt acc =
                  let next = max_n_wagner data threshold tabu_mgr data_generator cg n in
                  next :: acc
              in
    ...
    * function [b ()]. *)
    let max_n_randomized_p_wagner data threshold tabu_mgr cg nodes n p =
        let data_generator () = randomize nodes in
        max_n_dg_p_wagner data threshold tabu_mgr data_generator cg n p

    let max_n_mst_p_wagner data threshold tabu_mgr node_data cg nodes n p =
        let mst = mst node_data nodes in
        max_n_dg_p_wagner data threshold tabu_mgr mst cg n p

    let max_n_distances_p_wagner data threshold tabu_mgr cg nodes n p =
        let dord = distances_ordered nodes in
        max_n_dg_p_wagner data threshold tabu_mgr dord cg n p
        ...
        | `Build (n, build_meth, lst) ->
                ...
                if n < 1 then trees
                    ...
                    | `Wagner_Rnd (max_n, threshold, _, lst, tabu_mgr) ->
                            ...
                            let res = 
                                max_n_randomized_p_wagner data threshold tabu_mgr cg 
                                new_nodes max_n n
                            in
                    ...
                    | `Wagner_Ordered (max_n, threshold, keep_method, lst, tabu_mgr) ->
                            ...
                            let () =
                                ...
                                status in
                            let hd = 
                                max_n_wagner data threshold tabu_mgr (fun () -> new_nodes) cg 
                                max_n 
                            in
                            let () =
                                ...
                                status in
                            let tl = 
                                max_n_randomized_p_wagner data threshold tabu_mgr cg new_nodes 
                                max_n (n - 1)
                            in
                    ...
                    | `Wagner_Distances (max_n, threshold, keep_method, lst, tabu_mgr) ->
                            ...
                            let () =
                                ...
                                status in
                            let lst = 
                                max_n_distances_p_wagner data threshold tabu_mgr cg new_nodes 
                                max_n n 
                            in
                            let () = Status.finished status in
