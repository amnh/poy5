    ...
    *elist = (int *) NULL;
    ...
    if (plan->quadnearest) {
        ...
        if (call_quadnearest (ncount, dat, wcoord, plan->quadnearest, &kt, &td,
                              silent, rstate)) {
            fprintf (stderr, "/concorde/EDGEGEN/edgegen.c call_quadnearest failed\n");
            rval = 1;
            goto CLEANUP;
        }
    }
        ...
        CCrandstate *rstate)
{
    ...
    if (work_quadnearest (kt, ncount, nearnum, dat, wcoord, &tcount, &tlist,
                          silent, rstate)) {
        fprintf (stderr, "concorde/EDGEGEN/edgegen.c work_quadnearest failed\n");
        return 1;
    }
        ...
        int silent, CCrandstate *rstate)
{
    ...
    if ((norm & CC_NORM_BITS) == CC_KD_NORM_TYPE) {
        ...
        if (CCedgegen_junk_k_nearest (ncount, 2 * nearnum, dat, wcoord, 1,
                            ecount, elist, silent)) {
            fprintf (stderr, "concorde/EDGEGEN/edgegen.c CCedgegen_junk_k_nearest failed\n");
            return 1;
        }
    }
    return 0;
