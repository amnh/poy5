...
(**)            component_cost = Node.Standard.root_cost nnode;
    ...
    * sequences and likelihood only! *)
    let check_cost new_tree handle new_root =
        ...
        let real_cost = try begin
            let edge_visitor prev code acc =
                match Ptree.get_node code new_tree with
                | Tree.Single _
                | Tree.Leaf (_,_) -> acc
                | (Tree.Interior (_,par,a,b)) as v ->
                    let a,b = Tree.other_two_nbrs prev v in
                        let dist = (AllDirNode.AllDirF.distance_of_type
                            Node.has_to_single 
                            0.0
                            prev
                            (Ptree.get_node_data code new_tree)
                            (Ptree.get_node_data a new_tree)
                            (Ptree.get_node_data b new_tree)
                        ) in
                        acc +. dist
            in 
            let startinga,startingb =
                    let root = Ptree.get_component_root handle new_tree in
                    match root.Ptree.root_median with
                    | Some ((`Edge (a,b)),_) -> a,b
                    | Some _ 
                    | None -> begin match Ptree.get_node handle new_tree with
                                | Tree.Leaf (a,b)
                                | Tree.Interior(a,b,_,_) -> a,b
                                | Tree.Single _ -> raise Not_found
                              end
            in
            ...
            end with | Not_found -> 0.0
        in
        Printf.printf "REALCOST: %f\tROOTMINUS" real_cost;
        (* root costs *)
        let root_minus = 
