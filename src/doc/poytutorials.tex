...
homologous blocks (LCB) within the chromosomal sequences.  The float parameters that follow \poyargument{mauve} 
...
chromosomes, the data {\bf must} be transformed to \poyargument{fixed\_states} when using these Mauve aligners.
\item \texttt{transform(chromosome:(locus\_inversion:100,locus\_indel:\\(10,0.9)))}.  The \poycommand{trans\-form} 
command followed by the argument \poyargument{chromosome} signifies the conditions to be applied when calculating 
chromosome-level (medians).  The argument \poyargument{locus\_inversion} applies an inversion distance 
...
\item \texttt{swap(threshold:5.0)} The \poycommand{swap} command specifies that each of the trees is subjected to an alternating 
...
reported trees have the desired polarity.
\item \texttt{report("unann\_chrom\_diag.txt",diagnosis)}  The \poycommand{report} command, in combination with a file name and the argument
\poyargument{diagnosis}, outputs the optimal median states and edge values to the specified file (\texttt{unann\_chrom\_diag.txt}). 
...
The \poycommand{transform} follow\-ed by the argument \poyargument{annotated} specifies the conditions to be applied 
when calculating chromosome-level medians.  
The \poyargument{locus\_inversion} applies an inversion distance between chromosome loci with 
the integer value determining the rearrangement cost and using the default median solver, \poyargument{caprara}. 
The \poyargument{locus\_indel} specifies the indel costs 
for chromosomal segments, where the integer 10 setting the gap opening cost and the float 1.5 sets the gap extension 
cost.  Default values are applied to the \poyargument{circular},  \poyargument{median} and 
\poyargument{swap\_med} arguments in order to minimize the time require for these nested search options. 
...
reevaluated with values greater than these default values entered for the \poyargument{median} and \poyargument{swap\_med} 
arguments.)
...
\item \texttt{select()} Upon completion of branch swapping, this command retains only optimal and topologically 
unique trees; all other trees are discarded from memory. 
\item \texttt{report("ann\_chrom.diag",diagnosis)}  The \poycommand{report} command, in combination with a file name and 
...
applied when calculating genome-level HTUs (medians). \poyargument{locus\_breakpoint} 
...
\item \texttt{report("gen\_diag.pdf",graphdiagnosis,"gen\_cs.pdf", \\ graphconsensus)}  The \poycommand{report} command, 
...
by transforming user-defined \poyargument {custom\_alphabet} characters.  With this data type, insertion-deletion, 
