...
let () = SadmanOutput.register "Support" "$Revision: 3649 $"
...
module MakeNormal (Node : NodeSig.S with type other_n = Node.Standard.n)
  ...
  struct
    ...
    module TO = Ptree.Search (Node) (Edge) (TreeOps) 
        ...
        | count :: counts ->
            combine_counts 
                ~acc:(HashCount.fold
                        (fun k v map ->
                            let prev =
                                 try HashCount.find k map with Not_found -> 0 in
                             HashCount.add k (prev + v) map)
                        count
                        acc)
                counts

    let resample_support trees data queue iterations otus perturb search build outgroup counters =
        let n, data =
            let cnt = ref 0 in
            let chars = ref [] in
            Hashtbl.iter
                (fun _ code -> 
                    if Data.apply_boolean NonaddCS8.is_potentially_informative
                            AddCS.is_potentially_informative data code then
                        incr cnt 
                    else
                        chars := code :: !chars)
            ...
            !cnt, data
        in
        let new_data = match perturb with
            | `Bootstrap   -> Data.Sample.generate data `Bootstrap
            | `Jackknife m -> 
                if m > 100.0 || m < 0.0 then begin
                    Status.user_message Status.Error 
                        ("Illegal@ Jackknife@ sample@ size:@ The@ sample@ is@ "
                        ^"the@ percentage@ of@ characters@ that@ should@ be@ "
                        ^"sampled,@ therefore@ it's@ valid@ values@ are@ "
        ...
        let trees =
            ...
            Sexpr.map
                (fun x ->
                    TO.uppass (TO.downpass {x with Ptree.node_data = node_data}))
                trees
        in
        ...
        in
        let tree, _ = match Sexpr.to_list res_trees with
            | tree :: res_trees ->
                List.fold_left
    ...
    let bremer_to_sexpr ?edges tree =
        let visit (Tree.Edge (h, n) as edge) =
            ...
            if n_otus = 1 then
                ...
                let otu_set =
                    List.fold_left
                        (fun set elt ->
                            All_sets.Integers.add (Node.taxon_code elt) set)
        ...
        match edges with
            | Some edges ->
                let visit acc edge = match visit edge with
                    | None -> acc
                    | Some s -> s :: acc
                in
            ...
            | None ->
                let visit edge list = match visit edge with
                    ...
                    | Some s -> (Tree.Continue, s :: list)
                in
                let list_of_handle h acc =
                    Ptree.pre_order_edge_visit visit h tree acc
                in
    ...
    let bremer_item ?status original_tree tree build nodes (edge, otu_set) =
        let () = match status with
            ...
            | Some status ->
                Status.full_report
                    ~msg:("Making@ Bremer@ tree@ for@ clade@ with@ " ^
                        string_of_int (All_sets.Integers.cardinal otu_set) ^ "@ OTUs")
                    status
        in
        let treecost = Ptree.get_cost `Adjusted original_tree in
        if ndebug_bremer_best_costs then
            odebug ("Tree cost is " ^ string_of_float treecost);
        tree


    let minimum item =
        let comparison acc ((n, tree) as res) = match acc with
            | None                -> Some res
            | Some (n, othertree) ->
    ...
    let bremer_sexpr_to_support tree handle tree_cost sexpr =
        ...
        let rec descend parent id =
            match All_sets.IntegerMap.find id (tree.Ptree.tree.Tree.u_topo) with
                | Tree.Leaf (id, _) ->
                ...
                | (Tree.Interior (id, a, b, c) as node) ->
                    ...
                    let cost =
                        try let res = (List.assoc id alist) in
                            if res >= (infinity /. 2.) then Pervasives.infinity
                            else res -. tree_cost
                        with Not_found -> Pervasives.infinity
                    in
                    Methods.Node (cost, descend id left, descend id right)
                | Tree.Single _ -> assert false
        in
        match Ptree.get_node handle tree with
            | Tree.Single i ->
                let node = Ptree.get_node_data i tree in
                Methods.Leaf (Node.taxon_code node)
            | Tree.Leaf (id, parent) ->
                let node = Ptree.get_node_data id tree in
                let leaf = Methods.Leaf (Node.taxon_code node) in
                Methods.Node (Pervasives.infinity, leaf, descend id parent)
            | Tree.Interior (id, parent, left, right) ->
                let dec_left = descend id parent
                and dec_rght = descend parent id in
                Methods.Node (Pervasives.infinity, dec_left, dec_rght)
    ...
    let bremer_singletbr_nonadds tree handle cost param data emergency_queue =
        let search = param in
        ...
        let enumerate_list ?(starting=0) list =
            ...
            let ind =
                List.map
                    (fun id -> let c = !counter in incr counter; (id, c))
                    list
            in
            (ind, !counter)
        in
        ...
        let status =
            let n = num_leaves in
            (* (2n-3)! / ( 2^(n-2) * (n-2) ) *)
            let tmp1 =  Utl.factorial (2*n-3)
            and tmp2 = float_of_int (n-2) in
            let nbhd =  (float_of_int tmp1) /. ((2.**tmp2) *. tmp2) in
            Status.create "Bremer@ TBR@ neighborhood"
                          (Some (int_of_float nbhd)) "processing"
        in
        let () = Status.full_report status in
        let nbhood_count = ref 0 in
        let queue = 
            (best_vals, node_indices, starting, status, nbhood_count, orig_cost')
        in
        Status.full_report ~msg:"Performing@ neighborhood@ search" status;
        let set =
            let `LocalOptimum tabu = search in
            TreeSearch.sets tabu.Methods.tabu_join data (`Single tree')
        in
        let () =
            PTS.find_local_optimum ~queue data emergency_queue (`Single tree') set search
                --> ignore
        in
        Status.full_report ~msg:"Gathering results" status;
        let find_cost parent id = best_vals.(List.assoc id node_indices) in
        let rec descend parent id =
            match All_sets.IntegerMap.find id (tree.Ptree.tree.Tree.u_topo) with
            | Tree.Leaf (id, _) ->
                let node = Ptree.get_node_data id tree in
        ...
        let tree = match Ptree.get_node handle tree with
            ...
            | Tree.Leaf (id, parent) ->
                ...
                Methods.Node (Pervasives.infinity, leaf, descend id parent)
            | Tree.Interior (id, parent, left, right) ->
                let dec_left = descend id parent and dec_rght = descend parent id in
                Methods.Node (Pervasives.infinity, dec_left, dec_rght)
        in
    ...
    let bremer_support trees my_rank modul tree handle cost search build data nodes queue =
        ...
        match search_type.Methods.ss with
            ...
            | _ ->
                let sexpr =
                    if my_rank = 0 && modul = 1 then bremer_to_sexpr tree
                    else
                        let all_edges = Ptree.get_edges_tree tree in
                        let all_edges = List.sort Tree.EdgeComparator.compare all_edges in
                        let _, edges = 
                            List.fold_left
                                (fun (cnt, acc) edge ->
                ...
                in
                let sexpr =
                    bremer_run_sexpr trees data queue nodes search build tree sexpr
                in
    ...
    let support trees nodes meth data queue =
        let get_outgroup = function
            ...
            | Some v -> v
        in
        let sampler_function, iterations = match meth with
        ...
        in
        let st =
            Status.create "Calculating@ Support" (Some iterations) "Iterations"
        in
        let rec iterator cnt max f acc =
            Status.full_report ~adv:cnt st;
            if cnt > max then acc else iterator (cnt + 1) max f (f acc)
        in
        let res =
    ...
    let bremer_support build_trees my_rank modul nodes trees search_method build_method data queue =
        let f tree =
            ...
            bremer_support build_trees my_rank modul tree
                           (All_sets.Integers.choose (tree.Ptree.tree.Tree.handles))
                           cost search_method build_method data nodes queue 
        in
    ...
    let join_support_trees trees =
        let join2 (i1i, t1) (i2i, t2) =
            ...
            let rec join2 t1 t2 = match t1, t2 with
                | Methods.Leaf i, Methods.Leaf j when i = j -> Methods.Leaf i
                | Methods.Node (w1, t1l, t1r), Methods.Node (w2, t2l, t2r) ->
                    let left = join2 t1l t2l and rght = join2 t1r t2r
                    and numb = ((w1 *. i1) +. (w2 *. i2)) /. (i1 +. i2) in
                    Methods.Node (numb,left,rght)
                | _ ->
                    failwith "join_support_trees/join2: incompatible trees"
            in
    ...
    let generate_sets_for_bremer process_cost root data file =
        ...
        let for_brem =
            List.flatten
                (List.map (List.map (generate_sets process_cost data root)) trees)
        in
...
module Make (NodeH : NodeSig.S with type other_n = Node.Standard.n)
  ...
  struct
    ...
    let from_s_to_h =
        replace_contents TOH.downpass TOH.uppass NodeH.taxon_code 

    let from_h_to_s =
        replace_contents TOS.downpass TOS.uppass Node.Standard.taxon_code
