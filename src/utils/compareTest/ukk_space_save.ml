...
(*first char in seq must be gap*)
(*let seqarr2 = [|0;1;2;1;3;2;3|]
let seqarr1 = [|0;2;1;1;3;2|]
*)
...
(*    let lenX = Sequence.length x and lenY = Sequence.length y in*)
  let lenX = Array.length seqarr1 and lenY = Array.length seqarr2 in
    ...
    * but it has maxlen=lenY and I'm lazzzzzzy*)
    ...
    print_intlst base_diagonal_sizelst oc ;
    ...
    let get_addcost i j =
        (*Cost_matrix.Two_D.cost (Sequence.get seqarr1 i) (Sequence.get seqarr2 j)
        cost_matrix
...
(*update non-border cell, with i-1,j-1 and i-1,j and i,j-1*)
    let update_internal_cell ukkidx idxi idxj currentK diagonal_sizelst ukkArr =
        ...
        let costL = costfromL + gapcost in
        let costR = costfromR + gapcost in
...
(*test if we reject the currentK*)
    let ukktest thresholdT p diagonal_sizelst ukkArr =
        ...
        print_ukkcell ukkidx new_ukkArr.(ukkidx);
        (*if (cost<=thresholdT) then*)
            print_ukkArr new_ukkArr new_diagonal_sizelst newk;
        cost<=thresholdT,
        cost,
        max_gapnum,
    ...
    let rec increaseT newT diagonal_sizelst ukkArr =
        ...
        if testres=false then 
            if (new_cost < newT*2)||(newp>new_max_gapnum) then begin
                (*to do :get real align seq*)
                let alilst1,alilst2=ukk_traceback new_ukkArr
                new_diagonal_sizelst p in
                alilst1,alilst2,new_cost
            end
            else
            increaseT (newT*2) new_diagonal_sizelst new_ukkArr
        else begin
            let alilst1,alilst2=ukk_traceback new_ukkArr new_diagonal_sizelst p
            in
            alilst1,alilst2,new_cost
        end
        ;
...
let () =
    ...
    let seqarr1,seqarr2 = 
        if (Array.length seqarr1)<=(Array.length seqarr2) then
            ...
            seqarr2,seqarr1
    in
    let alilst1,alilst2,rescost = ukkonen_align seqarr1 seqarr2 oc in
    printf "%d%!" rescost;
    fprintf oc "end of ukk space save, rescost=%d,alied seq1/seq2 = \n%!" rescost;
    print_intlst alilst1 oc;
    print_intlst alilst2 oc;
    close_out oc
