...
let () = SadmanOutput.register "NonaddCS.nonadd_v" "$Revision: 2762 $"
...
external register_unmarshal : unit -> unit = "char_nonadd_CAML_register_unmarshal"

let () = register_unmarshal ()
...
external set_elt_code : ct -> int -> int -> unit = "char_nonadd_CAML_set_elt_code"
...
external distance_list : ct -> ct -> (int * float) list = "char_nonadd_CAML_distance_list"
...
external of_list_helper :
    ct -> (int * e * float) list -> int -> unit = "char_nonadd_CAML_of_list_helper"
...
let f_codes a codes =
    filter (fun (code, _, _) -> All_sets.Integers.mem code codes) 
           (fun code -> All_sets.Integers.mem code codes) 
           a

let f_codes_comp a codes =
    filter (fun (code, _, _) -> not (All_sets.Integers.mem code codes)) 
           (fun code -> All_sets.Integers.mem code codes) 
           a
        ...
        let rec filler item =
            if item = nelts then ()
                ...
                let elt =
                    ...
                    in
                    List.fold_left
                        (fun acc item -> acc lor (matcher item)) 0 elt
                in
...
let is_potentially_informative elts =
    let states =
        let make_list_set x = match x with
            | None   -> None
            | Some x ->
        ...
        List.fold_left
            (fun acc x -> match acc with
                ...
                | Some acc ->
                    begin match make_list_set x with
                        | None -> Some acc
    ...
    match states with
        | None        -> false 
        | Some states -> 0 = (All_sets.Integers.cardinal states)
...
let extract_elements_present elts =
    List.fold_right
        (fun x acc -> match x with
            | None -> acc
            | Some h -> (Nexus.File.static_state_to_list h) :: acc)
        elts []
...
let max_possible_cost elts =
    ...
    List.iter
        (List.iter
            (fun x -> 
                try let cnt = Hashtbl.find counter x in
                    Hashtbl.replace counter x (cnt + 1)
                with Not_found -> Hashtbl.add counter x 1))
        elts;
    let code, max =
        Hashtbl.fold
            (fun a b ((x, y) as acc) ->
                if y > b then acc else (a, b))
            counter (0, 0)
    in
    if max = 0 then 0.
    else 
...
let min_possible_cost (elts : Nexus.File.static_state list) =
    ...
    let states =
        let stateset =
            List.fold_left
                (fun acc y ->
                    List.fold_left
                        (fun acc x -> All_sets.Integers.add x acc) acc y)
    ...
    (* We define a function to filter the elements that are not covered by a
       particular element *)
    let filter c left =
        List.filter (fun x -> not (All_sets.Integers.mem c x)) left in
