...
(Edge : Edge.EdgeSig with type n = Node.n) 
    ...
    * roots *)
    ...
    let transform_nodes trees data nodes (trans : Methods.char_transform list) = 
        ...
        let st1 = Status.create "Transforming" (Some n_trans) " of transformations applied" in
        let apply_transformation trees ((pd,_) as acc) res =
            let n_chars = Hashtbl.length pd.Data.character_specs in
            let st2 = Status.create "Transforming" (Some n_chars) " of characters transformed" in
