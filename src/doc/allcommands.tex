...
\begin{command}{read}{}
	...
	\begin{statement}
        ...
        [orientation:\poybool, init3D:\poybool])}}
            ...
            breakinv data type is identical to custom\_alphabet data type.  If \texttt{init3D} is set to
            \texttt{true}, execution will be faster, but will require very large amounts of memory.} 
            {breakinv}
	...
	can be transformed into \poycommand{breakinv} using \poycommand{transform}.}
	 {customalphabet}
        ...
        \argumentdefinition{genome}{\obligatory{(\poystring list)}}
            ...
            allows for the translocation of segments of sequences between chromosome
            
            
            -level rearrangements specified by
            the argument
            \nccross{dynamic\_pam}{dynamicpam} of the command
...
\begin{command}{transform}{transformcommand}
	...
	\begin{arguments}
        ...
        \argumentdefinition{fixed\_states}{\optional{(\poystring, \poylident)}}
          ...
          where $i$ and $j$ are median states) to generate 
          graphical sequence maps between median states.  If \poylident is \texttt{ignore\_polymorphism},
          new potential median states will not be generated through resolution of sequence ambiguities
          (reducing execution time), otherwise
          new medians are generated as mentioned above.}
          {fixedstates}
...
%                           % \end{statement}
	...
	available at \texttt{http://www.cs.unm.edu/\~{}moret/GRAPPA/}.
	 ...
	 \argumentdefinition{breakinv\_to\_custom}{}
           ...
           {seqtochrom}
            ...
            The arguments of \poyargument{chromosome} define homologous blocks within unannotated chromosome sequences using
	...
	\begin{description}
		      ...
		      {Specifies the annotation method for unannotated chromosome sequences.
                        ...
                        float parameters quality of lcb, lcb coverage, minimum length of lcb (percent), 
                        maximum length of lcb (percentage). 
		     ...
		     \argumentdefinition{min\_seed\_length}{\obligatory{\poyint}}
	                    ...
	                    The default value is \texttt{100}}
                           ...
                           \argumentdefinition{max\_3d\_len}{\obligatory{\poyint}} 
                            {The three dimensional alignment for three chromosomes
                            is computed by separating into smaller three dimentional
                            alignments of short sequences whose lengths are smaller 
...
%		treated as linear and only a single set of median rearrangements are stored.}
		...
		Here, MAUVE genome alignment files will be generated with the names ``shrimp\_$i$\_$j$.alignment'' where $i$ and $j$ are
		median states.  Sequence ambiguities will not be resolved to generate additional medians beyond those determined by the
		data.}
