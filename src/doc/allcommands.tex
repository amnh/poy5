...
\begin{command}{calculate\_support}{calculatesupport}
    ...
    \begin{poydescription} 
        ...
        \poycommand{calculate\_support} command are optional and their order is
        arbitrary.  For examples of scripts implementing support measures see 
        ...
        and Bootstrap support values, as these sampling techniques do not require
        the presence of trees in memory. Therefore, it is possible to perform the
        sampling for support values \emph{before} the tree of interest has been found.
    \end{poydescription}
    ...
    \begin{statement}
        ...
        must be transformed into static characters using \poyargument{static\_approx}
        argument of the command~\ccross{transform} prior to executing
        \poycommand{calculate\_support}. Alternatively, an output file in the
        Hennig86 format can be generated based on an implied alignment
        using~\ccross{phastwinclad} that can subsequently be analyzed using other
        programs.  
    ...
    \begin{statement}
        ...
        \poycommand{calculate\_support}. See the description of the command
    ...
    \begin{arguments}
        ...
        \begin{argumentgroup}{Search strategy}
            ...
            \argumentdefinition{build}{}
                ...
                \poyargument{build} specifies the number of independent Wagner
                ...
                imported using the argument \poyargument{of\_file} of the command
                ...
                one is selected. If \poyargument{build} is omitted from the argument
                list of \poycommand{calculate\_support}, a single random addition
                Wagner tree per pseudoreplicate is built by default. This is
                equivalent to \poycommand{build(trees:1, randomized)}. See
                \nccross{build}{buildcommand} for a detailed discussion of arguments
                of the command \poycommand{build}.}
                {buildarg}
    ...
    \poydefaults{bremer,build(trees:1,randomized),swap(trees:1)}
        ...
        command \poycommand{calculate\_support()} does not have any effect.}
    ...
    \begin{arguments}
        \begin{argumentgroup}{Output class}
            \argumentdefinition{error}{}
                {Outputs the specified string as an error message
                (\texttt{stderr} in the \emph{flat} interface).}
                {errorecho}

            \argumentdefinition{info}{}
                {Outputs the specified string as an information message
                (\texttt{stderr} in the \emph{flat} interface).}
                {}

            \argumentdefinition{output}{\optional{\poystring}}
                {Reports a specified string (\texttt{stdout} in the \emph{flat}
                interface) to screen or file, if the filename string (enclosed
                in parentheses) is specified following \texttt{output} and
...
\begin{command}{fuse}{}
    ...
    \begin{poyexamples}
        ...
        \poyexample{fuse(swap(constraint))}
            ...
            \nccross{constraint}{swapconstraint} of the command \poycommand{swap}).}    
...
\begin{command}{help}{}
    ...
    \begin{arguments}
        \argumentdefinition{LIDENT}{}
            {Reports the description of the command, the name of which is
            specified by the LIDENT value.}
            {}

        \argumentdefinition{STRING}{}
            {Reports every occurrence in the help file of the expression
            specified by the string value.}
            {}
    \end{arguments}
    ...
    \begin{poyexamples}
        \poyexample{help(swap)}
            {Prints the description of the command \poycommand{swap} in the
            \emph{POY Output} window of the \emph{ncurses} interface or to the
            standard error in the \emph{flat} interface.}

        \poyexample{help("log")}
            {Finds every command with text containing the substring \texttt{log}
            and prints them in the \emph{POY Output} window of the \emph{ncurses}
...
\begin{command}{inspect}{}
    ...
    \begin{poydescription}
        ...
        \ccross{save}. If the description were not specified by the user, 
        \poycommand{inspect} reports that the description is not available. If
        the file is not a proper \poy file format, a message is printed in the
        \emph{POY Output} window of the \emph{ncurses} interface or to the
        standard error of the \emph{flat} interface.

        \poy files are not intended for permanent storage. They are recommended
        for temporary storage of a \poy session, checkpointing the current state
        of the search (to avoid losing data in case the computer or the program
        fails), or reporting bugs. \poy also automatically generates \poy files
        in cases of terminating errors (important exceptions are out-of-memory errors). 
    \end{poydescription}
...
\begin{command}{load}{}
    ...
    \begin{poydescription}
        Imports and inputs \poy files created by the command \poycommand{save}.
        The name of the file to be loaded is included in the string argument. All
        the information of the current \poy session will be replaced with the
        contents of the \poy file. If the file is not in proper \poy file format,
        an error message is printed in the \emph{POY Output} window of the
        \emph{ncurses} interface, or the standard error in the \emph{flat} interface.
        See the description of the command \ccross{save} on the \poy file and its usage.

        \poy files are not intended for permanent storage: they are recommended
        for temporary storage of a \poy session, checkpointing the current state
        of the search (to avoid losing data in case the computer or the program
        fails), or reporting bugs. \poy also automatically generates \poy files
        in cases of terminating errors (an important exception is out-of-memory
        error).
    \end{poydescription}
    ...
    \begin{poyexamples}
        \poyexample{load("initial\_search.poy")}
            ...
            \texttt{initial\_search.poy}, located in the current working directory.}
    \end{poyexamples}
...
\begin{command}{read}{}
    ...
    \begin{poydescription} 
        ...
        ``Sys\_error (``No such file or directory'')''.  \poycommand{read}
        ...
        single step. For example, \poycommand{read("*.fas*")} imports all files
        ...
        is achieved by typing \poycommand{read("BAP.*")}. Specifying a
        filename(s) is obligatory: an empty argument string,
        \poycommand{read()}, results in no data being read by \poy. The list of
        imported files and their content can be reported on screen or to a file
        using \poycommand{report(data)}.
    ...
    \begin{arguments}
        ...
        \begin{argumentgroup}{Basic data types}
            ...
            (except \poyargument{chromosome}, \poyargument{genome},
            \poyargument{custom\_alphabet} and \poyargument{breakinv}), as well
        ...
        \begin{argumentgroup}{Chromosome and genome type characters}          
            ...
            Chromosome sequences can be \poyargument {annotate} or unannotated.
            
            \argumentdefinition{annotate}{\obligatory{(\poystring list)}}
        ...
        \begin{argumentgroup}{Custom alphabet type characters}
            ...
            alphabet, as well as break inversion type.
             ...
             \argumentdefinition{breakinv}{\obligatory(\poystring list, tcm \obligatory(\poystring),{\optionall {\poylident list}})}
                ...
                with both \poyargument{breakinv} and \poyargument{custom\_alphabet} character types (see the argument~\nccross{custom\_alphabet}{customalphabet} below), 
            ...
            \begin{description}
                \argumentdefinition{orientation}{\obligatory \poybool}
                    ...
                    \poyargument{true} or \poyargument{false}.  It allows the user
            ...
            \argumentdefinition{custom\_alphabet}{(\obligatory\poystring list, tcm \obligatory(\poystring),{\optionall {\poylident list}})}
                ...
                Three optional arguments of \poyargument{custom\_alphabet} are
                possible ([\poylident list]): \poyargument {level};
                \poyargument{init3d}; and \poyargument{tiebreaker}.
...
%    \poyargument{custom\_to\_breakinv} within \poyargument{transform} (see~\ccross{transform}).
    ...
    \begin{poyexamples}
        ...
        \poyexample{read(custom\_alphabet:("my\_data",tcm:("alphabet"),tiebreaker: \\ last, orientation:false))}
...
\begin{command}{recover}{}
    ...
    \begin{poydescription}
        ...
        \poycommand{build}, \poycommand{calculate\_support}, or another \poycommand{swap}),
    ...
    \begin{poyexamples}
        \poyexample{recover()}{If the command \poycommand{swap} (executed
            ...
            for example, \poycommand{swap(tbr,recover)}, this command will restore
            the best trees recovered during swapping.}
    \end{poyexamples}
...
%                to reporting the distance.}{}
            ...
            \argumentdefinition{cross\_references}{\optional{identifiers\optional{STRING}}}
                ...
                \indent The argument \poyargument{cross\_references:all} generates
        ...
        \end{argumentgroup} 
         ...
         \begin{argumentgroup}{Exporting static homology data}
            ...
            \argumentdefinition{phastwinclad}{}
                ...
                characters must first be transformed into static characters
                using the \poycommand{transform} command (see example in tutorial \texttt{5.3}): 
                \begin{flushleft}
                    \poycommand{transform (all, (static\_approx))} \\
        ...
        \begin{argumentgroup}{Implied alignments}
            ...
            \argumentdefinition{implied\_alignments}{\optional{identifiers}}
                ...
                comma. This argument is synonymous with the argument \poyargument{ia}.}
                {impliedalignment}
        ...
        \begin{argumentgroup}{Trees}
            ...
            \argumentdefinition{supports}{\optional{argument}}
                ...
                in memory, for each one, the support values for each tree are reported. \poyargument{consensus}
...
%                        {cost}
    ...
    \poydefaults{data, diagnosis, trees}
        ...
        name enclosed in quotation marks, for example: \poycommand{report("filename")}.}
    ...
    \begin{poyexamples} 
        ...
        \poyexample{report("hennig.ss", phastwinclad, trees:(hennig, total))}
            ...
            Hennig86.  \index{general}{export!hennig}\index{general}{export!nona} \index{general}{export!tnt}}
...
\begin{command}{search}{}
    ...
    \begin{poydescription}
        ...
        are included in the set of trees available for the \poycommand{fuse} but are not swapped.
    \end{poydescription}

    \begin{arguments}

        \argumentdefinition{constraint}{\obligatory{\poystring}}
            {A complete description of this argument can be seen in
            \nccross{constraint}{buildconstraint} associated with the command
            \poycommand {build}.}
            {searchconstraint}

        \argumentdefinition{hits}{\obligatory{\poyint}}
            {Specifies the minimum number of times that the minimum cost must be
            reached before terminating the search.  The \poyargument{hits}
        ...
        \argumentdefinition{memory}{\obligatory{\poylident:\poyfloat}}
            ...
            value expresses the units of memory (\poyargument{gb} for Gigabytes
            and \poyargument{mb} for Megabytes), whereas the float value
        ...
        \argumentdefinition{min\_time}{\obligatory{\poyfloat:\poyfloat:\poyfloat}}
            ...
            \poyargument{hits} is specified but the actual cost of the tree is
    ...
    \poydefaults{max\_time:0:1:0, min\_time:0:1:0, memory:gb:2}
        ...
        the value of \poyargument{max\_time}, the search will be terminated
        after one hour.]}
...
%                        
        ...
        \begin{argumentgroup}{Cost calculation}
            ...
            \argumentdefinition{iterative}{\obligatory{\poylident:\optional \poyint}}
                ...
                \indent If the argument value is \poyargument{exact}, this method improves the tree
                ...
                strategy is to apply  \poyargument{iterative} at the very end of
...
\begin{command}{swap}{swapcommand}
    ...
    \begin{arguments}
        ...
        \begin{argumentgroup}{Join method}
            ...
            \argumentdefinition{constraint}{\optional{\poyint | (depth:\poyint, file:\poystring)}}
                ...
                \poyargument{depth} set to \texttt{0} (no maximum distance
                is specified).}
                {swapconstraint}
    ...
    \poydefaults{trees:1, alternate, threshold:0, bfs}
...
% Chromosome and genome transformation methods} -------------------------------
        ...
        \begin{argumentgroup}{Chromosome and genome transformation methods}
            ...
            \argumentdefinition{chromosome}{\obligatory{([argument list])}}
                ...
                events: (i.e. \poyargument{locus\_inversion}, \poyargument{locus\_dcj}, or 
                \poyargument{locus\_breakpoint}, and \poyargument {locus\_indel}), 
            ...
            \begin{statement}
                Note that the arguments \poyargument{locus\_breakpoint} and
                \poyargument {chrom\_breakpoint} cannot be used simultaneously
            ...
            \argumentdefinition{locus\_breakpoint}{\obligatory{\poyint}}
                ...
                conjunction with \poyargument{locus\_inversion}.or \poyargument{locus\_dcj}. The default
   ...
   %\end{figure}
                ...
                \argumentdefinition{seq\_to\_chrom}{\obligatory{([argument list])}}   
                    ...
                    \poyargument{locus\_inversion}, \poyargument{locus\_dcj}, and 
                    \poyargument{locus\_indel}) can be specified by the argument.}
                    {seqtochrom}
