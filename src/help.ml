...
many more possible tree topologies and generates a diversity of
...
computed as frequencies of clades recovered in strict consensus
trees built in each resampling iteration. The consensus trees are
based on best trees recovered in each replicate with zero-length
branches collapsed. All the arguments of calculate_support
...
\"Sys_error (\"No such file or directory\")\". The filename must match exactly.
read automatically detects the type of the 
...
fragment 0 contained in the file chel.aln.select(terminals,missing:50) 
...
terminals is automatically reported on screen.select(optimal) 
