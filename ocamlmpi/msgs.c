...
/* $Id: msgs.c 2 2004-06-22 15:40:20Z andres $ */
...
/* This function was added by Andres Varon. */
                   ...
                   value dest, value tag, value vcomm)
{
  ...
  Begin_root(vcomm)             /* prevent deallocation of communicator */
    ...
    /* This also allocates the buffer */
    enter_blocking_section();
    MPI_Isend(buffer, len, MPI_BYTE, Int_val(dest), Int_val(tag), comm, &req);
    leave_blocking_section();
  End_roots();
  stat_free(buffer);
  MPI_Test(&req, &flag, &status);
 /* MPI_Request_free(req);*/
  return Val_bool(flag);
}
...
/* Non blocking probe for pending messages and determine length */
/* Andres Varon */
