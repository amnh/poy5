...
\begin{arguments}
        ...
        \argumentdefinition{optimize:(model:([options]),branch:([options]))}{}
            {Specifies when we optimize the likelihood model and how we optimize
            the branches. These options are also available in the
            \poycommand{fuse()} and \poycommand{swap()} commands.

            \begin{description}

                \argumentdefinition{model:(never)}{}
                    {Do not optimize the model during the build. After the build a
                    round of optimizations will occur.}
                    {modelnever}
                ...
                \argumentdefinition{model:(threshold:FLOAT)}{}
                    {Optimize the model if the cost of the current models join is
                    better by \poyargument{FLOAT} times the current best cost.}
                    {modelthreshold}

                \argumentdefinition{model:(max\_count:INT)}{}
                    {Optimize the model after INT joins. This can be useful during
                    the build process where we do improve the model, but where after
                    every join it can be time consuming.}
                    {maxcount}
                ...
                \argumentdefinition{branch:(never)}{}
                    ...
                    There is a final round of optimization after building.}
                    {branchnever}
...
\begin{command}{fuse}{}
    ...
    \begin{arguments}
        ...
        \argumentdefinition{optimize:(model:([options]),branch:([options]))}{}
            {Specifies the likelihood model and how the branches are optimized. 
            These options are also available in the
            \poycommand{build()} and \poycommand{swap()} commands.
            ...
            \begin{description}
                ...
                \argumentdefinition{model:(max\_count:INT)}{}
                    {Optimize the model after INT joins. This can be useful during
                    the build process where the model is  improved, but  after which,
                    every join  can be time consuming.}
                    {maxcount}
                    
                \argumentdefinition{branch:(never)}{}
                    {Do not optimize the branches during the build process. Estimates are made
                    based on the proportion of sites that undergo a transformation.
                    There is a final round of optimization after building.}
                    {branchnever}
...
\begin{command}{read}{}
	...
	\begin{poydescription} 
        ...
        with \texttt{biv}. \poycommand{read("*.ss")} imports all files with
	...
	\begin{statement}
        ...
        \end{statement}
         ...
         \argumentdefinition{annotated}{\obligatory{(\poystring list)}}
            ...
            %the argument \nccross{dynamic\_pam}{dynamicpam} of%
        ...
        [orientation:\poybool%, init3D:\poybool%
            ...
            rearrangement events. %specified using \poycommand{dynamic\_pam()}%. 
            Syntactically, breakinv data type is identical to custom\_alphabet data type.  %If \texttt{init3D} 	   
           % is set to \texttt{true}, execution will be faster, but will require very large amounts of memory.
           } 
            {breakinv}
            ...
            %change tutorial--or add new one?%
	...
	An optional argument of \poyargument{custom\_alphabet} is \poyargument{orientation}
	%and \poyargument{init3D), both of
	...
	%\poyargument{init3D:false}. The default option is \texttt{true}. If the optional argument \poylident is \texttt{prealigned}, the sequences are
	 ...
	 {customalphabet}
        ...
        \argumentdefinition{genome}{\obligatory{(\poystring list)}}
           ...
           the command \nccross{transform}{transformcommand}. Chromosome
            ...
            %distance threshold levels specified by the argument \nccross{chrom\_hom}{chromhom}
            the command \nccross{transform}{transformcommand}
	...
	\begin{poyexamples}
        ...
        %of custom-alphabet characters is considered and prior calculation of medians for their triplets
	...
	\begin{argumentgroup}{Terminals and characters}
        ...
        \begin{argumentgroup}{Exporting static homology data}
            ...
            \argumentdefinition{nexus}{}
                ...
                using the \poycommand{transform} command (see the Hennig86 example in tutorial \texttt{5.2}): 
                \begin{flushleft}
...
\begin{command}{swap}{swapcommand}
	...
	\begin{arguments}
	    ...
	    \begin{argumentgroup}{Likelihood Optimization}
            {Specifies when we optimize the likelihood model and how we optimize
            the branches furing a swap. \poycommand{fuse()} and \poycommand{build()} commands.}
            \argumentdefinition{optimize:(model:([options]),branch:([options]))}
            For a sample scripts using likelihood optimization see tutorials \texttt{5.10} and \texttt{5.11}.
            {}
                {\begin{description}
...
\begin{command}{transform}{transformcommand}
	...
	\end{statement}
            ...
            {transformtcm}
              ...
              \argumentdefinition{tcm}{\obligatory{(\poystring, level:\poyint)}}
        ...
        \begin{argumentgroup}{Likelihood transformation methods}
            ...
            \argumentdefinition{rates}{}
                ...
                \begin{description}
                    ...
                    \argumentdefinition{gamma[:(N,$\alpha$)]}{}
                    ...
                    \argumentdefinition{theta[:(N,$\alpha$,\%]}{}
...
%                           % \end{statement}
	...
	Concord TSP Solver available at \texttt{http://www.tsp.gatech.edu/concorde/}.
	 	...
	 	\argumentdefinition{annotate}{\obligatory{(\poylident,\poyfloat,\poyfloat,\poyfloat,\poyfloat)}}
            ...
            The arguments of \poyargument{chromosome} define homologous blocks within unannotated chromosome sequences by
            ...
            %(\poyargument{min\_seed\_length}, \poyargument{min\_loci\_len}, and \poyargument{min\_rearranged\_len}); specify thecost
            (i.e. \poyargument{locus\_inversion}, \poyargument{locus\_dcj}, or \poyargument{locus\_breakpoint}, and
             \poyargument {locus\_indel}), take into account whether the chromosome is linear or circular (\poyargument{circular}), and
             implement a number of 	heuristic procedures to accelerate computations (\poyargument{median}, \poyargument{swap\_med}, 
             and \poyargument {med\_approx}).  Under default settings, the pairwise distance between two chromosome segments or two 
           ...
           {Transforms \poyargument{custom\_alphabet} characters into the \poyargument{breakinv} character type to allow for 
            ...
            example, when \poyargument{custom\_alphabet} characters are used to define a sequence of individual genes and one is 
 		...
 		\argumentdefinition{newkkonen}{\optional{\poylident}}
		  ...
		  %\argumentdefinition{chrom\_hom}{\obligatory{(\poyfloat)}}
           ...
           {Transforms nucleotide type data into \\ chromosome type data to allow
            ...
            \poyargument{locus\_inversion}, \poyargument{locus\_dcj}, and \poyargument{locus\_indel}) can be 
            specified by the argument.}
            %\poyargument{dynamic\_pam} If no \poyargument{dynamic\_pam} values are specified, its default values are applied%
           {seqtochrom}
	...
	\begin{poyexamples} 
		...
		\poyexample{transform(search\_based:(("mycustom\_char1.fas","mycustom\_extra1.fas"),
		...
		\poyexample{transform(search\_based:"mycustom\_extra.fas")}
		...
		%with the minimum length of identical contiguous sequence fragments  which form the seeds of homologous blocks set at  
		%\texttt{15} nucleotides 
		and the locus-level gap opening cost is set at \texttt{50} with a gap extension cost at \texttt{1.0}.}
              	
		 \poyexample{transform(level:2)}
