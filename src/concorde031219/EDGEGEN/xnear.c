    ...
    for (n = 0; n < ncount; n++) {
        if (usex) {
            ...
            if (CCedgegen_junk_node_k_nearest (dat, wcoord, n, num, ncount,
                                               list)) {
                fprintf (stderr, "concorde/EDGEGEN/xnear.c junk_node_k_nearest_failed\n");
                rval = 1;
                goto CLEANUP;
            }
        }
        ...
        int nearnum, int ncount, int *list)
{
    ...
    //Note: we will do insert() n+ncount-n-1=ncount-1 times on nearlist
    //
    for (j = n - 1; j >= 0; j--) {
    ...
    if (ntotal < nearnum) {
        fprintf (stderr, 
                "WARNING(/concorde/EDGEGEN/xnear.c): \
                ntotal=%d,There do not exist %d neighbors\n",
                 ntotal,nearnum);
        for (i = ntotal; i < nearnum; i++)
            list[i] = -1;
        return 1;
