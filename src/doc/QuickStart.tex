...
\begin{flushleft}
	...
	\end{minipage}
		...
		\begin{itemize}
           ...
           [Note: The \poy developers encountered problems when using MPICH2 1.4.1.p1, therefore it is advised that the 
...
\begin{quote}
\commandstyle{poy -w /Users/username/docs/poyfiles mol.poy}
\end{quote}
...
\begin{quote}
\commandstyle{mpirun -np 8 /POY\_bins/poy5a-mpi mol.poy}\\
\end{quote}
...
\begin{center}
\renewcommand{\arraystretch}{1.5}
\begin{tabular}{p{3.0cm}  p{1.4cm}  p{2.40cm}  p{2.40cm}  p{0.75cm}} 
\hline
...
\end{description}   
       ...
       \hangindent=1cm	\emph{Parsimony Optimality Criterion}
        ...
        below and in the \emph{Chromosome and genome transformation methods} section of  ~\ccross{transform}.
...
\setlength{\parindent}{0.5cm}        
	...
	\item [Chromosome and Mauve Parameters] \emph{Chromosome} characters are multi-locus 
		...
		the \emph{Chromosome and genome transformation methods} of the command \poycommand{transform}, 
		see ~\nccross{transform}{transform}.  It is possible to submit either \emph{Annotated} (by selection of the 
...
parentheses (e.g. \poycommand{read()}) always follow the command name. An exhaustive discussion of \poy command 
...
other types of input files see \commandstyle{help(read)}.)
...
A typical output of \commandstyle{cross\_references} command is shown in Figure ~\ref{fig:crossref}. The reported cross 
...
are for a particular command, use either \commandstyle{help()} command with the command name of interest inserted in 
...
Using different combinations of \commandstyle{swap()} arguments allows designing a  large number of search 
...
This command \commandstyle{select()}, is another multifunctional command the arguments of which are also used to 
...
\emph{POY5 Mail Group} is hosted  by Google at
	\begin{center}
	\url{http://groups.google.com/group/poy}  
	\end{center}
