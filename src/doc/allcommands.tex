...
builds within each constraint. Constraining a tree is useful in 
hypothesis testing.}
{buildconstraint}
...
of the \poylident specified. \hl{...} 

If \texttt{single}, the default, a collapse will
occur if the length single assignment on a branch is zero; 

if \texttt{min}
uses final up pass assignment calculation minimum between those---a 
lot of collapsing; 

if texttt{max} uses final up pass assignment calculation 
maximum , minimal collapsing. 

The boolean value \texttt{true}---zero 
length branches will be collapsed if the \poylident value is \texttt{true}, 
and will not if \texttt{false}.}
{}
...
\argumentdefinition{graphtrees}{\optional{collapse}}
{Same as the \poyargument{trees} \hl{...} Output the diagnosis in PDF format. The PDF is compressed, and
contains the trees and links to see the diagnosis of each vertex
in the tree.}
{graphtrees}
...
The default choice method is \texttt{first}. [Note: Levels greater than the default 
